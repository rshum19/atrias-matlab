...
classdef IMUParams
	properties
		align_gyro_tol  = 1e-4 % Motion tolerance during alignment, rads/millisecond
		align_accel_tol = .02  % Motion tolerance during alignment, Gs
		align_time_ms   = 31 * 1000; % Alignment time, in milliseconds
		earth_rot_rate  = 7.292115e-5 * .001; % Earth's rotation rate, rad/millisecond. From WolframAlpha
		bad_data_tol    = 20                  % The number of cycles (sequence counter updates) we'll tolerate missing.
