classdef  controller3DwalkNrunNhop5 < Controller
...
%                 +0*(ldev*sign(x_st)+0*(x_st>0));%*(dx<dx_des)*walkCmd;
            ...
            k_fp_h_v=k_fp_h_v-.03*abs(obj.dx_des)-1.5*lpo;
            k_fp_h_e=k_fp_h_e +0*lpo;
            
            
            k_fp_l_v=k_fp_l_v-.035*abs(obj.dx_des)+1*lpo;
            k_fp_l_e=k_fp_l_e+.035*abs(obj.dx_des);
            ...
            yss = obj.yss-0*.7*lpo-.015*abs(obj.dx_des);
...
%                     l0sw=clamp(l0sw+0*.01*abs(obj.dx)+0*.01*(obj.dx-obj.dx_des),l0sw,.96);
                    ...
                    if abs(x_f2f_d)>step_max
                        ...
                        l0sw=clamp(l0sw+.1*(abs(x_f2f_d)-step_max),l0sw,.96);
...
%                     l0sw=clamp(l0sw+.2*(abs(obj.dx)-abs(obj.dx_des))*(abs(obj.dx)>abs(obj.dx_des)),l0sw,.96);
                    ...
                    if t<(.5*obj.T)
                        [q21d,dq21d]=spline3(t,.5*obj.T,obj.q21d0,0,acos(l0sw),0);
                    else
...
%             
            ...
            if obj.isSim
                ...
                dx_cmd = 0*clamp(-.3*floor(obj.ti/2.5),-2.4,2.4) -0*1.3*(obj.ti>2)-0*1.1*(obj.ti>6.5)-0*(obj.ti>8); dy_cmd = 0*(obj.ti>2);
                if obj.gaitMode==GaitMode_S.Hop
                    dx_cmd=0;
                    dy_cmd=0;
                end
