classdef  controller3DwalkNrunNhop2 < Controller
...
%             lpo=lpo+(.015-0*.003*(-1)^obj.RefLeg)*abs(obj.dx_des)*(sign(obj.dx)==sign(obj.dx_des));
            ...
            lpo_v=.015*abs(obj.dx_des)*(sign(obj.dx)==sign(obj.dx_des));
...
%                 +0*(ldev*sign(x_st)+0*(x_st>0));%*(dx<dx_des)*walkCmd;
            ...
            ys0=obj.ys0-0*.2*lpo+.01*(obj.dx_des)-0*.7*deltaLx +.2*lpo;
...
%                     l0sw=clamp(l0sw+0*.01*abs(obj.dx)+0*.01*(obj.dx-obj.dx_des),l0sw,.96);
                    ...
                    step_max=.4+clamp(.2*(abs(obj.dx_des)-1.2)*(abs(obj.dx_des)>1.2),0,.25);
...
%                         l0sw=clamp(l0sw+.025*abs(obj.dx),l0sw,.96);
                     ...
                     lmin=l0-obj.lret+.02*obj.dx_des+0*1.7*lpo;
...
%                     
                    ...
                    if t<.2*obj.T
                        s_u=spline3(t, .2*obj.T, .5, 0, 1, 0);
                    elseif t<.5*obj.T
                        s_u=1;
                    elseif t<.8*obj.T
                        s_u=spline3(t-.5*obj.T, .3*obj.T, 1, 0, 1+.15*abs(obj.dx_des)+10*lpo, 0);
                    elseif t<obj.T
                        s_u=spline3(t-.8*obj.T, .2*obj.T, 1+.15*abs(obj.dx_des)+10*lpo, 0, .5, 0);
                    else
                        s_u=.5;
                    end
