function [eStop, u, userOut, x_st, dxy_f] = controller3DwalkNrun(q, dq, userIn)
...
lpo_x=.015*abs(dx_des)-.01*(dx-dx_des)*sign(dx_des);
...
k_fp_l_v=clamp(k_fp_l_v-.03*dx_des,.05,k_fp_l_v);


yss=clamp(yss+0*max(1*deltaLx,0),0,yss);
...
qTc=clamp(c_yaw*((2*(q(12)-yaw_d)+0*.005*dq(12))/(1+8*deltaLx)),-.15,.15);
