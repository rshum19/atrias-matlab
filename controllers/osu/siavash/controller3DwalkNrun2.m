function [eStop, u, userOut, x_st, dxy_f] = controller3DwalkNrun2(q, dq, userIn)
...
persistent x y tp xc yc ycp xcp xcpp ycpp dxp dyp dzp t0 RefLeg Tcp Tcpp...
    ...
    lFp rFp x_fpt0 x_f2f T y_fp y_fp2 f2f0 xI yI xId yId xd yd ycppp Tcppp yaw_c yaw_cp
...
if RefLeg==RIGHT 
    lpo=clamp(lpoR,lpoR_p-3*.01/1.5*(ti-tp),lpoR_p+3*.01/1.5*(ti-tp));
    lpoR_p=lpo;
else
    lpo=clamp(lpoL,lpoL_p-3*.01/1.5*(ti-tp),lpoL_p+3*.01/1.5*(ti-tp));
    lpoL_p=lpo;
end
...
lpo_x=0*.015*abs(dx_des)-.05*(dx-dx_des)*sign(dx_des);%-clamp(.3*(-1)^RefLeg*(q(12)-0),-.03,.03);
...
k_fp_h_v=k_fp_h_v - clamp(.05*abs(dx_des)+.5*max(deltaLx,0),0,.1);

k_fp_l_v=clamp(k_fp_l_v -.05*abs(dx_des),.05,k_fp_l_v);
k_fp_l_e=clamp(k_fp_l_e+0*.03*abs(dx_des),k_fp_l_e,.3);


yss=clamp(yss+0*1*deltaLx,.05,.2);
% xss=xss+c_yaw(1+0*30*deltaLx)*(0*yaw_c+.3*q(12))*(ti>t_stb);%-.01*(ti>t_stb);
...
qTc=clamp(c_yaw*(5*yaw_cp+.2*(1*(q(12)-yaw_d)+.005*dq(12))/(1+8*deltaLx)),-.15,.15);
...
%         fc_h=fc;
        ...
        if RefLeg==RIGHT
            u(3)=clamp( -u0_hip +kp_hip*(fc*(roll-roll_d))+kd_hip*(fc*droll),-.7*fc*F_l-u0_hip,.7*fc*F_l+u0_hip)...
                +(1-c_h)*(1-fc_h)*(kp_hip*(qh1d0-q(9))+kd_hip*(-dq(9)));
            u(6)=u0_hip+((q_hip_d-q(10))*(1-fc2)*c_h+fc2*(roll-roll_d))*kp_hip + ((dq_hip_d - dq(10))*(1-fc2)*c_h+fc2*droll)*kd_hip;
             deltaq1=(qh1d0-q(9));
        else
            u(6)=clamp(u0_hip+kp_hip*(fc*(roll-roll_d))+kd_hip*(fc*droll),-.7*fc*F_l-u0_hip,.7*fc*F_l+u0_hip)...
                +(1-c_h)*(1-fc_h)*(kp_hip*(qh1d0-q(10))+kd_hip*(-dq(10)));
            u(3)=-u0_hip+((q_hip_d-q(9))*(1-fc2)*c_h+fc2*(roll-roll_d))*kp_hip + ((dq_hip_d - dq(9))*(1-fc2)*c_h+fc2*droll)*kd_hip;
            deltaq1=(qh1d0-q(9));
        end
