classdef  controller3DwalkNrunNhop7 < Controller
...
%             ldev=.015*(abs(dx_des)-obj.vs)*walkCmd-0*clamp(0.03*q(12),-01,.01)*(-1)^obj.RefLeg*(obj.ti>obj.t_stb);
            ...
            l0=obj.l0+clamp(.005*abs(obj.dx_des),0,.01);%+0*(obj.ti>obj.t_stb)*clamp(0*-.06*q(12)*(-1)^obj.RefLeg-.003*(-1)^obj.RefLeg,-.04,.04);
...
%                 +0*(ldev*sign(x_st)+0*(x_st>0));%*(dx<dx_des)*walkCmd;
            ...
            k_fp_h_v=k_fp_h_v-obj.kv_h*abs(obj.dx_des)-1.5*lpo;
            k_fp_h_e=k_fp_h_e +0*lpo;
            
            
            k_fp_l_v=k_fp_l_v-obj.kv_l*abs(obj.dx_des)+1*lpo;
            k_fp_l_e=k_fp_l_e+obj.kv_l*abs(obj.dx_des);
...
%                     l0sw=clamp(l0sw+0*.01*abs(obj.dx)+0*.01*(obj.dx-obj.dx_des),l0sw,.96);
                    ...
                    step_max=.4+clamp(.13*(abs(obj.dx_des)-1)*(abs(obj.dx_des)>1)+.15*(abs(obj.dx)-abs(obj.dx_des))*(abs(obj.dx)>abs(obj.dx_des)),0,.45);
...
%                     [q22d,dq22d]=spline3(t,.1*obj.T,obj.q22d0,obj.dq22d0,-uT/obj.k+(theta1+theta2)/2,(dtheta1+dtheta2)/2);
                    ...
                    elseif t<.5*obj.T
                        s_u=1;
                    elseif t<.8*obj.T
                        s_u=spline3(t-.5*obj.T, .3*obj.T, 1, 0, 1+.1*abs(obj.dx_des)+10*lpo, 0);
                    elseif t<obj.T
                        s_u=spline3(t-.8*obj.T, .2*obj.T, 1+.1*abs(obj.dx_des)+10*lpo, 0, .5, 0);
                    else
                        s_u=.5;
                    end
...
%                 obj.dx_des obj.dy_des lpo obj.sampleInterval obj.q_hip_d_s obj.rF obj.lF dx_uf_p dy_uf_p u x_fp']; %obj.runTime] %
...
%                     obj.l_ret = 0.12;
                ...
                case GaitMode_S.Dynamic
                    t_c=.5; dx_max=1.2;  dy_max = 0.2; obj.lpo=0;
...
%             
            ...
            if obj.isSim
                t_c=.5+0*.5*(obj.ti>3)+0*.5*(obj.ti>8);
                dx_cmd = 0*clamp(-.3*floor(obj.ti/2.5),-2.4,2.4) -0*1.4*(obj.ti>2)-0*1*(obj.ti>7)-0*(obj.ti>8); dy_cmd = 0*(obj.ti>2);
                if obj.gaitMode==GaitMode_S.Hop
                    dx_cmd=0;
                    dy_cmd=0;
                end
