classdef  controller3DwalkNrunNhop8 < Controller
    ...
    properties
        state@double=1;
%         sT@double=[1.15 1 1.1 1];
        sT@double=[1.13 1.08 1.17 .9];
...
%             
            ...
            obj.lpo_p=lpo;
%             lpo=lpo+(.015-0*.003*(-1)^obj.RefLeg)*abs(obj.dx_des)*(sign(obj.dx)==sign(obj.dx_des));
            

		    obj.T=obj.T0-obj.k_time*(abs(obj.dx_des)-1.4)*(abs(obj.dx_des)>1.4);
            
            
            lpo=spline3(t-.4*obj.T,.25*obj.T,0,0,lpo*(1-1.5*(abs(obj.dx)-.2)*(abs(obj.dx)>.2)),0);
...
%             ldev=.015*(abs(dx_des)-obj.vs)*walkCmd-0*clamp(0.03*q(12),-01,.01)*(-1)^obj.RefLeg*(obj.ti>obj.t_stb);
            ...
            l0=obj.l0+0*clamp(.005*abs(obj.dx_des),0,.01);%+0*(obj.ti>obj.t_stb)*clamp(0*-.06*q(12)*(-1)^obj.RefLeg-.003*(-1)^obj.RefLeg,-.04,.04);
            ...
            q22=(thetam1+thetam2)/2;
            q24=(thetam3+thetam4)/2;
...
%             l_yaw=0*clamp(50*x_st*((yaw_c-yaw_cp)+0*dq(12)+0*.2*q(12))*(-1)^obj.RefLeg*(obj.ti>obj.t_stb),-lpo/4,lpo/4);
            ...
            deltaLx=l_yaw+...
                ...
                lpo+(lpo_v)*(t>obj.T/2)+(-0*lpo)*(t<=obj.T/2); %*(dz>0)...%*(dz<0)...
%                 +0*(ldev*sign(x_st)+0*(x_st>0));%*(dx<dx_des)*walkCmd;
            ...
            k_fp_h_v=k_fp_h_v-2*obj.kv_h*abs(obj.dx_des)-1.5*lpo;
            k_fp_h_e=k_fp_h_e - obj.kv_h*abs(obj.dx_des) +0*lpo;
            
            
            k_fp_l_v=k_fp_l_v-4*obj.kv_l*abs(obj.dx_des)+1*lpo;
            k_fp_l_e=k_fp_l_e+0*obj.kv_l*abs(obj.dx_des);
            ...
            yss = obj.yss-0*.7*lpo-obj.kv_yss*abs(obj.dx_des);
            
            if obj.dx_des>0
                ys0=obj.ys0-0*.2*lpo+obj.ys0v*(obj.dx_des)-0*.7*deltaLx +.2*lpo;
            else
                ys0=obj.ys0-0*.2*lpo+.5*obj.ys0v*(obj.dx_des)-0*.7*deltaLx +.2*lpo;
            end
...
%             qTc=0*clamp(c_yaw*(1.5*yaw_cp+0*(1.5*(q(12)-yaw_d)+.005*dq(12))/(1+0*8*deltaLx)),-.15,.15);
            ...
            qTc=-0*.4*obj.dx_des*pi/180*(1+sin(2*pi*t/obj.T-pi/2));
            dqTc=-0*.4*obj.dx_des*pi/180*(2*pi/obj.T)*cos(2*pi*t/obj.T-pi/2);
            ddqTc=0*.4*obj.dx_des*pi/180*(2*pi/obj.T)^2*sin(2*pi*t/obj.T-pi/2);
            xs0=obj.xs0+0*qTc/10;
            ...
            x_fp=[k_fp_l_v*obj.dx; k_fp_h_v*obj.dy]+[k_fp_l_e; k_fp_h_e].*[1;1].*[obj.dx-obj.dx_des; obj.dy-obj.dy_des]+[xs;ys]...
                ...
                obj.kD*[0;0].*[obj.dx-obj.xcp/obj.Tcp; obj.dy-obj.ycp/obj.Tcp]+...
                obj.kI*[1;0].*[obj.x-obj.x_des; 0]+...
                obj.kD*[0;1].*[obj.dx-obj.dxp;obj.dy-obj.dypp];
...
%                     l0sw=clamp(l0sw+0*.01*abs(obj.dx)+0*.01*(obj.dx-obj.dx_des),l0sw,.96);
                    ...
                    step_max=4*.4+clamp(.13*(abs(obj.dx_des)-1)*(abs(obj.dx_des)>1)+.15*(abs(obj.dx)-abs(obj.dx_des))*(abs(obj.dx)>abs(obj.dx_des)),0,.45);
                    if abs(x_f2f_d)>step_max
%                         deltaLx=clamp(deltaLx-.15*(abs(x_f2f_d)-step_max),-.06,.96-l0);
                        l0sw=clamp(l0sw+.1*(abs(x_f2f_d)-step_max)+0*.01,l0sw,.96);
%                         x_f2f_d=step_max*sign(x_f2f_d);
                    end
                    
                     lmin=clamp(l0-obj.lret-.02*abs(obj.dx_des)+0*1.7*lpo,.55,.8);
...
%                     end
                    ...
                    if t<(obj.k_t_po*obj.T)
                        [q21d,dq21d]=spline3(t,.5*obj.T,obj.q21d0,obj.dq21d0,acos(l0sw),0);
                    elseif t<(1*obj.T)
...
%                     if ctrltest==2
%                         T1=40*Tsw/100;
                        T1=obj.legRetPerc*Tsw/100;
%                         T2=80*Tsw/100;
                        T2=(80+0*3*abs(dx_des))*Tsw/100;
%                         T3=60*Tsw/100;
                        T3=45/100*Tsw;  % smaller T3 make it hop!
%                         T4=90*Tsw/100;
                        T4=clamp((100-0*7*obj.dx_des)/100-0*8*lpo,.83,1)*Tsw;
%                     else
%                         T1=1*Tsw/100;
%                         T4=81*Tsw/100;
%                         T2=70*Tsw/100;
...
%                     
                    ...
                    tsw0=.15*obj.T-clamp(.05*abs(obj.dx_des),0,.15)*obj.T;
                    ...
                    F_l_d=2*obj.k*(((theta2-theta1)/2)-q21d)/sqrtc(1-cos((theta2-theta1)/2)^2);
                    uT=1*clamp(-0*(obj.kp_leg*(q24d-(thetam3+thetam4)/2)+obj.kd_leg*(dq24d-(dthetam3+dthetam4)/2))/50+0*F_l*.1*cos(theta-pitch)/(1+.1/ll1*sin(theta-pitch))+ddqTc+k_torso*kdT*(dqTc-dpitch)+k_torso*kpT*(qTc-pitch),-.7*fc*F_l,.7*fc*F_l);%+(u3+u4)/n/IT;
                    uT2=1*clamp(0*F_l2*.1*cos(theta_sw-pitch)/(1+.1/ll2*sin(theta_sw-pitch))+ddqTc+k_torso*kdT*(dqTc-dpitch)+k_torso*kpT*(qTc-pitch),-.7*fc2*F_l2,.7*fc2*F_l2);%+(u3+u4)/n/IT;
                    
                    
%                     [q22d,dq22d]=spline3(t,.1*obj.T,obj.q22d0,obj.dq22d0,-uT/obj.k+(theta1+theta2)/2,(dtheta1+dtheta2)/2);
                    q22d=-uT/obj.k+(theta1+theta2)/2;
                    dq22d=(dtheta1+dtheta2)/2;
                    
                    
                    if t<.1*obj.T
                        s_u=spline3(t, .1*obj.T, .5, 0, 1, 0);
                    elseif t<.5*obj.T
                        s_u=1;
                    elseif t<.8*obj.T
                        s_u=spline3(t-.5*obj.T, .3*obj.T, 1, 0, 1+.05*abs(obj.dx_des)+10*lpo, 0);
                    elseif t<obj.T
                        s_u=spline3(t-.8*obj.T, .2*obj.T, 1+.05*abs(obj.dx_des)+10*lpo, 0, .5, 0);
                    else
                        s_u=.5;
                    end
...
%                         ([1 1 1 1].*([(q22d-q21d-thetam1) (q22d+q21d-thetam2) (q24d-q23d-thetam3)-(1-fc)*fc2*uT2/obj.k (q24d+q23d-thetam4)-(1-fc)*fc2*uT2/obj.k])+...
...
%                         obj.kd_leg*[1 1 1 1].*([dq22d-dq21d dq22d+dq21d dq24d-dq23d dq24d+dq23d]-[dthetam1 dthetam2 dthetam3 dthetam4]);
                  ...
                  if obj.torso_ctrl==true
                    u([2 1 5 4])=obj.kp_leg*[s_u s_u .5 .5].*...
                        [1 1 1 1].*([(q22d-q21d-thetam1) (q22d+q21d-thetam2) (q24d-q23d-thetam3) (q24d+q23d-thetam4)-(1-fc)*fc2*uT2/obj.k])+...
                        obj.kd_leg*[1 1 1 1].*([dq22d-dq21d dq22d+dq21d dq24d-dq23d dq24d+dq23d]-[dthetam1 dthetam2 dthetam3 dthetam4])...
                        -0*[fc*uT fc*uT (1-fc)*fc2*uT2 (1-fc)*fc2*uT2]...
                        +obj.torso_ff*obj.k/2*(theta1+theta2-thetam1-thetam2)*[fc fc 0 0];
                  else
                    u([2 1 5 4])=obj.kp_leg*[s_u s_u .5 .5].*...
                        [1 1 1 1].*([(q22-q21d-thetam1) (q22+q21d-thetam2) (q24d-q23d-thetam3) (q24d+q23d-thetam4)])+...
                        obj.kd_leg*[1 1 1 1].*([dq22d-dq21d dq22d+dq21d dq24d-dq23d dq24d+dq23d]-[dthetam1 dthetam2 dthetam3 dthetam4])...
                        -[fc*uT fc*uT (1-fc)*fc2*uT2 (1-fc)*fc2*uT2];
...
%                 obj.dx_des obj.dy_des lpo obj.sampleInterval obj.q_hip_d_s obj.rF obj.lF dx_uf_p dy_uf_p u x_fp' deltaLx]; %obj.runTime] %
%             
...
%                     obj.l_ret = 0.12;
                ...
                case GaitMode_S.Dynamic
                    t_c=1.5; dx_max=1.25;  dy_max = 0.2; obj.lpo=0;
...
%                     obj.l0_leg = 0.91;
%                     obj.l_ret = 0.2;
                    t_c = 2; dx_max = 0.75; dy_max = 0.35; obj.lpo=0;
            end % switch
...
%             
            ...
            if obj.isSim
                t_c=1+0*.5*(obj.ti>3)+0*.5*(obj.ti>8);
                dx_cmd = 0*clamp(-.3*floor(obj.ti/2.5),-2.8,2.8) -0*1.4*(obj.ti>2)-0*.7*(obj.ti>7)-0*(obj.ti>8); dy_cmd = 0*(obj.ti>2);
                if obj.gaitMode==GaitMode_S.Hop
                    dx_cmd=0;
                    dy_cmd=0;
                end
