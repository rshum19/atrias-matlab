classdef  siavash_walkNrunNhop < Controller
    ...
    properties
        ...
        k_fp_h_v@double=.2;             % Lateral feedforward gain (k_fp_h_v)
        k_fp_l_v@double=.25;            % Sagittal feedforward gain (k_fp_h_v)
        
        k_fp_h_e@double=.1;             % Lateral proportional gain (k_fp_h_e)
        ...
        kD@double=0;                    % Foot placement derivative gain (kD)
        
        
        xss@double=0;                   % Nominal sagittal toe to toe distance  (xss)
        yss@double=.16;                 % Nominal lateral toe to toe distance (yss)
        
        ys0v@double=-.01;               % CoM Correction factor (ys0v)
        
        kv_yss@double=0.015;            % Lateral deviation correction factor (kv_yss)
        kv_h=0.005;                     % Lateral gain correction factor (kv_h)
        kv_l=0.005;                     % Sagittal gain correction factor (kv_h)
    ...
    properties (Access = protected)
        ...
        out@double=zeros(1,45); %output vector
    ...
    methods
        ...
        function u = userStep(obj, q, dq)
            ...
            % TODO: Delete after robot testing
            %             RefLegF2=obj.RefLeg;
            ...
            % Leg gains
            k_fp_l_v=obj.k_fp_l_v-4*obj.kv_l*abs(obj.dx_des)+1*lpo;
            ...
            yss = obj.yss-obj.kv_yss*abs(obj.dx_des);
            ...
            u([2 1 5 4])=obj.kp_leg*[s_u s_u .5 .5].*...
                ...
                +obj.k/2*(theta1+theta2-thetam1-thetam2)*[fc fc 0 0];
            ...
            % Smooth hip angle
            q_hip_d=clamp(c_h_p*spline3(t-t_s_i, T, obj.q_hip_d_s, 0, q_hip_d, 0),.13*(-1)^obj.RefLeg,-.28*(-1)^obj.RefLeg);
            
            % Torso roll control
            % Scaling for aerial phase
            k_hop=1;
            if obj.RefLeg==obj.RIGHT
                % Stance leg
                % PD control on roll considering friction
                u30=clamp( (obj.kp_hip*(k_torso*fc*(roll-roll_d))+...
                    obj.kd_hip*(k_torso*fc*droll)),-.7*fc*F_l,.7*fc*F_l);
                % Including hopping
                u(3)=u30...
                    +k_hop*(1-c_h_p)*(1-fc_h)*(obj.kp_hip*(qh1d0-q(9))+obj.kd_hip*(-dq(9)))...
                    -40*(1-cos(2*pi/T*t))/2;
                
                % Swing leg
                % Use swing leg to help roll control
                if  t>=t_s_i
                    u(6)=((q_hip_d-q(10))*(1-fc2)*c_h_p+k_torso*fc2*(roll-roll_d))*obj.kp_hip +...
                        ((dq_hip_d - dq(10))*(1-fc2)*c_h_d+k_torso*fc2*droll)*obj.kd_hip;
                ...
                % Stance leg
                % PD control on roll considering friction
                u60=clamp( ( obj.kp_hip*(k_torso*fc*(roll-roll_d))+ obj.kd_hip*(k_torso*fc*droll)),-.7*fc*F_l,.7*fc*F_l);
                % Including hopping
                u(6)=u60...
                    +k_hop*(1-c_h_p)*(1-fc_h)*(obj.kp_hip*(qh1d0-q(10))+obj.kd_hip*(-dq(10)))...
                    +40*(1-cos(2*pi/T*t))/2;
                
                % Swing leg
                % Use swing leg to help roll control
                if  t>=t_s_i
                    u(3)=((q_hip_d-q(9))*(1-fc2)*c_h_p+k_torso*fc2*(roll-roll_d))*obj.kp_hip +...
                        ((dq_hip_d - dq(9))*(1-fc2)*c_h_d+k_torso*fc2*droll)*obj.kd_hip;
                else % Angular momentum control
        ...
        function parsePS3Controller(obj)
            ...
            if obj.isSim
                t_c=2+0*.5*(obj.ti>3)+0*.5*(obj.ti>8);
                dx_cmd = 0*clamp(-.3*floor(obj.ti/2.5),-2.8,2.8) -2.8*(obj.ti>2)-0*.7*(obj.ti>7)-0*(obj.ti>8); dy_cmd = 0*(obj.ti>2);
