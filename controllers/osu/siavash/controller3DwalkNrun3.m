function [eStop, u, userOut, dx_des, dxy_f] = controller3DwalkNrun3(q, dq, userIn)
...
persistent x y tp xc yc ycp xcp xcpp ycpp dxp dyp dzp t0 RefLeg Tcp Tcpp...
    dx_fp dy_fp dz_fp dx_des_p dy_des_p lpoL_p lpoR_p q22d0...
    lFp rFp x_fpt0 x_f2f T y_fp y_fp2 f2f0 xI yI xId yId xd yd ycppp Tcppp yaw_c yaw_cp yaw_cpp
...
% kp_hip=kp_hip-300+300*(-1)^RefLeg;
...
%     lpo=.01*(ti>t_stb)+.01*(ti>7)+.01*(ti>10)+.01*(ti>13)+0*.01*(ti>17);
%     lpo=clamp(.01/3*(ti-t_stb)*(ti>t_stb),0,.05);
    lpoR=0*.04*(ti>t_stb);
    lpoL=0*.04*(ti>t_stb);
...
%     if ctrltest==2
        ...
        c_yaw=clamp(userIn(22),-2,2);
        lpo=clamp(userIn(23),-.08,.08);
        lpoR=lpo;
        lpoL=lpo;
        
        xss=clamp(userIn(24),-.08,.08);
        
        k_hop=clamp(userIn(25),0,1);
%     else
%         k_fp_h_e=userIn(7);
%         k_fp_l_e=userIn(8);
%         T0=userIn(9);
...
T0=clamp(T0-0*.05*(dx_des-vs)*(dx_des>vs),.2,T0);
...
if RefLeg==RIGHT 
    lpo=clamp(lpoR,lpoR_p-.01/1.5*(ti-tp),lpoR_p+.01/1.5*(ti-tp));
    lpoR_p=lpo;
else
    lpo=clamp(lpoL,lpoL_p-.01/1.5*(ti-tp),lpoL_p+.01/1.5*(ti-tp));
    lpoL_p=lpo;
end
...
dx_des0=dx_des;
% dx_des=dx_des+clamp((c_yaw*q(12)+0*3*dq(12)+0*.16*(1+0*dx_fp))*(-1)^RefLeg,-.25,.25); %*(ti>t_stb);
% dx_des=dx_des+c_yaw*(yaw_cp+.3*q(12))*(-1)^RefLeg;

% dx_des=dx_des-2*(-1)^RefLeg;

if abs(dx_des)>.1*vs || abs(dy_des)>.2
...
% dx_des=clamp(dx_des-(-1)^RefLeg*.1,dx_des-.1,dx_des+.1);
...
ldev=.015*(abs(dx_des)-vs)*walkCmd-0*clamp(0.03*q(12),-01,.01)*(-1)^RefLeg*(ti>t_stb);
...
if abs(dx-dx_des)>10*(0.2*abs(dx_des)+.4) || abs(dy-dy_des)>1*(0.2*dy_des+.4) || kI==0 %|| ~walkCmd 
    ts=ti;
    x=0;
    y=0;
    xc=0;
...
lpo_x=.015*abs(dx_des)-(kyaw_l+.02)*(dx-dx_des)*sign(dx_des);%-clamp(.3*(-1)^RefLeg*(q(12)-0),-.03,.03);

lpo_x=(.015+kyaw_l)*abs(dx_des);

l_yaw=0*clamp(50*x_st*((yaw_c-yaw_cp)+0*dq(12)+0*.2*q(12))*(-1)^RefLeg*(ti>t_stb),-lpo/4,lpo/4);
deltaLx=l_yaw+...
    lpo_x*(x_st>0)- lpo_x/3*(x_st<=0)+...
    (lpo*(t>T/2)+0*lpo*(t<=T/2))...%*(dz>0)...%*(dz<0)...
    +0*(ldev*sign(x_st)+0*(x_st>0));%*(dx<dx_des)*walkCmd;


% k_fp_h_v=k_fp_h_v - clamp(1*max(deltaLx,0),0,.1);

k_fp_h_v=k_fp_h_v - clamp(2*.05*abs(dx_des)+3*max(deltaLx,0),0,.1);
k_fp_h_e=k_fp_h_e - 0*clamp(.5*.05*abs(dx_des)+.5*max(deltaLx,0),0,.06);



k_fp_l_v=clamp(k_fp_l_v -(kyaw_x+.05)*abs(dx_des)-0*.05*(dx_des-dx)*sign(dx_des),.05,k_fp_l_v);
k_fp_l_e=clamp(k_fp_l_e-0*.05*abs(dx_des),.05,k_fp_l_e);


% yss=clamp(yss-.5*max(deltaLx,0)-.01*abs(dx_des),.05,.2);
% xss=xss+1*(1+0*30*deltaLx)*(0*yaw_c+.3*q(12))*(ti>t_stb);%-.01*(ti>t_stb);

xss=xss-0*.04*abs(dx_des)+0*2*deltaLx;

ys0=ys0+0*.02*abs(dx_des)-0*.7*deltaLx;


T0=T0-clamp((0*.02+c_yaw*(0*(yaw_d-q(12))-0*yaw_c-2*yaw_cp)*(ti>t_stb))*(-1)^RefLeg,-.04,.04);
% (.01-.01*(ti>5)-.04*(ti>10))*(-1)^RefLeg
...
x_fp=[k_fp_l_v*dx; k_fp_h_v*dy]+[k_fp_l_e; k_fp_h_e].*[dx-dx_des; dy-dy_des]+[xs;ys]...
    +[1;0].*[clamp(kI*.05*(1+abs(dx_des))*(x-0*xc-xd),-.05,.05); clamp(2*kI*.05*(1+dy_des)*(y-0*yc-yd),-.05,.05)]...%*walkCmd...
    +0*clamp(.0005*[0;1].*[xcp/Tcp+xcpp/Tcpp-2*dx_des; ycp/Tcp+ycpp/Tcpp-2*dy_des],-.02,.02)*walkCmd...
    +0*[0; -.015]*walkCmd...
    -kD*[0;1].*clamp([.1*(xcp/Tcp-dx); .1*(ycp/Tcp-dy)],-.05,.05)...%*walkCmd...
...
switch state
    ...
    case 1 % WALK --------------------------------------------------------
        ...
        x_f2f_d = clamp(x_fp(1)-(cos(theta2)+cos(theta1))/2, -0.4, 0.4);
        
%         l_yaw=-2*x_st*dq(12)*(-1)^RefLeg*(ti>t_stb);
...
%         end
        ...
        u([2 1 5 4])=kp_leg*[2^.5 2^.5 1/2^.5 1/2^.5]'.*...
            ([1 1 1 1]'.*([(q22d-q21d) (q22d+q21d) q24d-q23d-fc2*uT2/k q24d+q23d-fc2*uT2/k]'-[thetam1 thetam2 thetam3 thetam4]')+...
            0*(1-c_h)*(1-fc_h)*[q22d0-q21d-thetam1 q22d0+q21d-thetam2 0 0]')+...
...
%     if ctrltest==4
        ...
        qh1d0=clamp(asin(clamp(y_fp2/sqrt(ll1^2+d^2),-1,1))-atan2(d,ll1)*(-1)^RefLeg-roll,-.13*(-1)^RefLeg,.28*(-1)^RefLeg);
%     else
...
%         fc_h=fc;
        ...
        if RefLeg==RIGHT
            u(3)=clamp( -u0_hip +kp_hip*(fc*(roll-roll_d))+kd_hip*(fc*droll),-.7*fc*F_l-u0_hip,.7*fc*F_l+u0_hip)...
                +k_hop*(1-c_h)*(1-fc_h)*(kp_hip*(qh1d0-q(9))+kd_hip*(-dq(9)));
            u(6)=u0_hip_sw+((q_hip_d-q(10))*(1-fc2)*c_h+fc2*(roll-roll_d))*kp_hip + ((dq_hip_d - dq(10))*(1-fc2)*c_h+fc2*droll)*kd_hip;
             deltaq1=(qh1d0-q(9));
             
             u(3)=u(3)+0*100;
        else
            u(6)=clamp( u0_hip+kp_hip*(fc*(roll-roll_d))+kd_hip*(fc*droll),-.7*fc*F_l-u0_hip,.7*fc*F_l+u0_hip)...
                +k_hop*(1-c_h)*(1-fc_h)*(kp_hip*(qh1d0-q(10))+kd_hip*(-dq(10)));
            u(3)=-u0_hip_sw+((q_hip_d-q(9))*(1-fc2)*c_h+fc2*(roll-roll_d))*kp_hip + ((dq_hip_d - dq(9))*(1-fc2)*c_h+fc2*droll)*kd_hip;
            deltaq1=(qh1d0-q(9));
        ...
        %-clamp(kpT*(yaw_d-q(12))-kdT*dq(12),-200,200)*(-1)^RefLeg
...
if (ti-t0)>=T+0*.5*(abs(dx_des)-vs)*abs(dx_des)*walkCmd*(abs(dx-dx_des)<.5*abs(dx_des)) || ((ti-t0)>T/2 && RefLegF2~=RefLeg)
    Tcppp=Tcpp;
    Tcpp=Tcp;
    Tcp=ti-t0;
    t0=ti;
